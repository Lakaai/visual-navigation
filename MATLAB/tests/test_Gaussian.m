...
S = [ ...
    10, 11, 12, 13, 14; ...
     ...
     0,  0,  0, 23, 24; ...
     0,  0,  0,  0, 25];
runGaussianCases(testCase, mu, S);
end
...
%% Helper functions
function runGaussianCases(testCase, mu, S)
P = S.'*S;
% Xi = qr(S^{-T})
...
runGaussianCase(testCase, p, mu, P, eta, Lambda);

p = Gaussian.fromMoment(mu, P);
runGaussianCase(testCase, p, mu, P, eta, Lambda);

p = Gaussian.fromInfo(eta, Lambda);
runGaussianCase(testCase, p, mu, P, eta, Lambda);

p = Gaussian.fromSqrtInfo(nu, Xi);
runGaussianCase(testCase, p, mu, P, eta, Lambda);
end

function runGaussianCase(testCase, p, mu_expected, P_expected, eta_expected, Lambda_expected)
n = p.dim();
