...
public:
    ...
    static Gaussian fromSqrtInfo(const Eigen::VectorX<Scalar> & nu, const Eigen::MatrixX<Scalar> & Xi)
    {
        ...
        out.S_ = Xi.template triangularView<Eigen::Upper>().transpose().solve(
            Eigen::MatrixX<Scalar>::Identity(Xi.rows(), Xi.cols())
        );
     ...
     */
    virtual Eigen::MatrixX<Scalar> infoMat() const override // inverse of cov 
    {
        ...
        return S_.template triangularView<Eigen::Upper>().solve(
            S_.template triangularView<Eigen::Upper>().transpose().solve(
                Eigen::MatrixX<Scalar>::Identity(S_.rows(), S_.cols())
            )
        );
    }
    ...
    Gaussian marginal(const IndexType & idx) const
    {
        ...
        // Select the elements of the mean vector corresponding to the desired marginal
        out.mu_ = mu_(idx);
        
        // Select the columns of S_ corresponding to the desired marginal
        Eigen::MatrixX<Scalar> S_marginal = S_(Eigen::all, idx);
        
        // Perform QR decomposition
        Eigen::HouseholderQR<Eigen::Ref<Eigen::MatrixX<Scalar>>> qr(S_marginal);
        out.S_ = qr.matrixQR().topRows(idx.size()).template triangularView<Eigen::Upper>();
...
{
    Eigen::MatrixX<Scalar> SS(S_.rows(), idxB.size() + idxA.size());
    SS << S_(Eigen::all, idxB), S_(Eigen::all, idxA);
    
    Eigen::HouseholderQR<Eigen::MatrixX<Scalar>> qr(SS);
    Eigen::MatrixX<Scalar> R = qr.matrixQR().template triangularView<Eigen::Upper>();

    Eigen::Index nA = idxA.size();
    Eigen::Index nB = idxB.size();

    Eigen::MatrixX<Scalar> SBB = R.topLeftCorner(nB, nB);
    Eigen::MatrixX<Scalar> SBA = R.topRightCorner(nB, nA);
    Eigen::MatrixX<Scalar> SAA = R.bottomRightCorner(nA, nA);

    Eigen::VectorX<Scalar> muA = mu_(idxA);
    Eigen::VectorX<Scalar> muB = mu_(idxB);

    Eigen::MatrixX<Scalar> Sc = SAA;
    Eigen::VectorX<Scalar> muc = muA + SBA.transpose() * SBB.template triangularView<Eigen::Upper>().transpose().solve(xB - muB);

    return Gaussian::fromSqrtMoment(muc, Sc);
}
    ...
    Gaussian affineTransform(Func h) const
    {
        ...
        // return out;
        Eigen::MatrixX<Scalar> C;
        Eigen::VectorX<Scalar> y = h(mu_, C);
        
        // Compute S * C^T
        Eigen::MatrixX<Scalar> SC = S_ * C.transpose();
        
        // Perform QR decomposition of SC^T
        Eigen::HouseholderQR<Eigen::MatrixX<Scalar>> qr(SC);
        
        // Extract the upper triangular part R, ensuring it's square
        Eigen::MatrixX<Scalar> R = qr.matrixQR().topLeftCorner(C.rows(), C.rows()).template triangularView<Eigen::Upper>();
        
        // Create the transformed Gaussian
        return Gaussian::fromSqrtMoment(y, R);
    }
    ...
    virtual Scalar log(const Eigen::VectorX<Scalar> & x) const override
    {
        ...
        // Solve S^T * w = x - μ for w
        Eigen::VectorX<Scalar> w = S_.transpose().template triangularView<Eigen::Lower>().solve(x - mu_);
        
        Scalar logPDF = -0.5 * w.squaredNorm() 
                        - 0.5 * dim() * std::log(2 * M_PI) 
                        - S_.diagonal().array().abs().log().sum();
    ...
    Scalar log(const Eigen::VectorX<Scalar> & x, Eigen::VectorX<Scalar> & g) const
    {
        ...
        // Solve S^T * w = x - μ for w
        Eigen::VectorX<Scalar> w = S_.transpose().template triangularView<Eigen::Lower>().solve(x - mu_);
             // Compute gradient: g = -S^(-1) * w
        g = -S_.template triangularView<Eigen::Upper>().solve(w);
    ...
    // Solve S^T * w = x - μ for w
        ...
        // Compute Hessian: H = -S^(-1) * S^(-T)
        H = -S_.template triangularView<Eigen::Upper>().solve(
        S_.transpose().template triangularView<Eigen::Lower>().solve(Eigen::MatrixX<Scalar>::Identity(dim(), dim())));
    ...
    virtual bool isWithinConfidenceRegion(const Eigen::VectorX<Scalar> & x, double nSigma = 3.0) const override
    {
        ...
        // Solve S.'*z = x - mu (triangular forward substitution)
        Eigen::VectorX<Scalar> z = S_.transpose().template triangularView<Eigen::Lower>().solve(x - mu_);

        // Check if the point is within the confidence region
        return z.squaredNorm() <= r2;
    ...
    Eigen::Matrix4<Scalar> quadricSurface(double nSigma = 3.0) const
    {
        ...
        // NEW CODE HERE
        assert(this->dim() == 3 && "Expected trivariate Gaussian");

        Scalar c = 2 * this->normcdf(nSigma) - 1;
        Scalar r2 = this->chi2inv(c, this->dim());

        Eigen::Matrix3<Scalar> Sti = S_.transpose().template triangularView<Eigen::Lower>().solve(Eigen::Matrix3<Scalar>::Identity());
        Eigen::Vector3<Scalar> z = S_.transpose().template triangularView<Eigen::Lower>().solve(mu_);
        Eigen::Vector3<Scalar> y = -S_.template triangularView<Eigen::Upper>().solve(z);

        Eigen::Matrix4<Scalar> Q;
        Q.template topLeftCorner<3,3>() = Sti.transpose() * Sti;
        Q.template topRightCorner<3,1>() = y;
        Q.template bottomLeftCorner<1,3>() = y.transpose();
        Q(3,3) = z.dot(z) - r2;
