% Minimise f(x) using trust-region quasi-Newton BFGS method (square-root Hessian)
function [x, g, Xi, ret] = BFGSTrustSqrt(costFunc, x, Xi, verbosity)
...
for i = 1:maxIterations
    ...
    p = funcmin.trsSqrt(Xi, g, Delta);    % minimise 0.5*p.'*Xi.'*Xi*p + g.'*p subject to ||Xi*p|| <= Delta
    z = Xi*p;
    
    pg = p.'*g;
    LambdaSq = -pg;  % The Newton decrement squared is g.'*inv(H)*g = p.'*H*p = p.'*Xi.'*Xi*p
    if verbosity == 3
