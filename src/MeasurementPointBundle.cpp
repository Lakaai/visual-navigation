...
MeasurementPointBundle::MeasurementPointBundle(double time, const Eigen::Matrix<double, 2, Eigen::Dynamic> & Y, const Camera & camera)
    : MeasurementVisualNav(time, camera)
    , Y_(Y)
...
MeasurementVisualNav * MeasurementPointBundle::clone() const
{
...
{
    SystemVisualNavPointLandmarks & systemVisualNav = dynamic_cast<SystemVisualNavPointLandmarks &>(system);
    ...
    // std::vector<std::size_t> idxLandmarks_(systemVisualNav.numberLandmarks());
    // std::cout << "idxlandmarks size in update" << idxLandmarks_.size() << std::endl;
    // std::iota(idxLandmarks_.begin(), idxLandmarks_.end(), 0);
    // const std::vector<int> & idxFeatures_ = associate(systemVisualNav, idxLandmarks_);
    ...
    //     if (idxFeatures_[j] < 0) {
    //         systemVisualNav.incrementFailedObservations(idxLandmarks_[j]);
    //         if (systemVisualNav.getFailedObservations(idxLandmarks_[j]) > 10) { // Adjust threshold as needed
    //             systemVisualNav.removeLandmark(idxLandmarks_[j]);
    //         }
    //     } else {
    //         systemVisualNav.resetFailedObservations(idxLandmarks_[j]);
    //     }
    // }   
    ...
    //     Eigen::Vector3d rCNn = systemVisualNav.cameraPosition(camera_, systemVisualNav.density.mean());
    //     Eigen::Matrix3d Rnc = systemVisualNav.cameraOrientation(camera_, systemVisualNav.density.mean());
    ...
    //     systemVisualNav.initializeNewLandmark(landmarkPosition);
    //     newLandmarksAdded++;
    // }
