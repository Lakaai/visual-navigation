...
MeasurementUniqueTagBundle::MeasurementUniqueTagBundle(double time, const Eigen::Matrix<double, 8, Eigen::Dynamic> & Y, const Camera & camera, const std::vector<int> & markerIds)
...
double MeasurementUniqueTagBundle::logLikelihood(const Eigen::VectorXd & x, const SystemEstimator & system, Eigen::VectorXd & g) const
{
    ...
    autodiff::dual fdual;
    g = gradient(&MeasurementUniqueTagBundle::logLikelihood<autodiff::dual>, autodiff::wrt(x_dual), autodiff::at(this, x_dual, system), fdual);
...
Eigen::Vector2d MeasurementUniqueTagBundle::predictFeature(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const SystemSLAM & system, std::size_t idxLandmark) const
{
    // Set elements of J
    // TODO: Lab 7 (optional)
    using autodiff::dual;
...
Eigen::VectorXd MeasurementUniqueTagBundle::predictFeatureBundle(const Eigen::VectorXd & x, Eigen::MatrixXd & J, const SystemSLAM & system, const std::vector<std::size_t> & idxLandmarks) const
{
...
void MeasurementUniqueTagBundle::initializeNewLandmark(SystemSLAM & system, int markerId, const Eigen::Matrix4d & tagPose)
{
...
Eigen::Matrix4d MeasurementUniqueTagBundle::getTagPoseCamera(const std::vector<cv::Point2f> &corners, const Camera &camera, double tagLength) const
{
...
Eigen::Matrix4d MeasurementUniqueTagBundle::getTagPoseWorld(const std::vector<cv::Point2f> &corners, const Camera &camera, double tagLength, const Pose<double> &bodyPoseWorld) const
{
    // Define 3D coordinates of tag corners in tag's local frame
    std::vector<cv::Point3f> objectPoints = {
        cv::Point3f(-tagLength/2,  tagLength/2, 0),
