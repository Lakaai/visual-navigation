...
std::size_t SystemVisualNavPointLandmarks::numberLandmarks() const
{
    return (density.dim() - 12)/3;
}

std::size_t SystemVisualNavPointLandmarks::landmarkPositionIndex(std::size_t idxLandmark) const
{
    assert(idxLandmark < numberLandmarks());
    return 12 + 3*idxLandmark;    
}

void SystemVisualNavPointLandmarks::incrementFailedObservations(std::size_t idxLandmark) {
    failedObservations_[idxLandmark]++;
}

int SystemVisualNavPointLandmarks::getFailedObservations(std::size_t idxLandmark) const {
    auto it = failedObservations_.find(idxLandmark);
    return (it != failedObservations_.end()) ? it->second : 0;
}

void SystemVisualNavPointLandmarks::resetFailedObservations(std::size_t idxLandmark) {
    failedObservations_[idxLandmark] = 0;
}

void SystemVisualNavPointLandmarks::removeLandmark(std::size_t idxLandmark) {
...
void SystemVisualNavPointLandmarks::initializeNewLandmark(const Eigen::Vector3d& position) {
