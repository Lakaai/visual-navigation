...
{
private:
    // This will stored the landmarkIds as they are added to the state vector, we can then refer to this list when performing data association
    std::vector<int> landmarkIds_;                  
...
public:
    ...
    virtual GaussianInfo<double> processNoiseDensity(double dt, int scenario) const override;
    ...
    virtual void addLandmark(int markerId);
    virtual int getLandmarkId(std::size_t idxLandmark) const;
    virtual std::size_t getLandmarkIndex(int markerId) const;
