...
double snn(const SystemSLAM & system, const GaussianInfo<double> & featureBundleDensity, const std::vector<std::size_t> & idxLandmarks, const Eigen::Matrix<double, 2, Eigen::Dynamic> & Y, const Camera & camera, std::vector<int> & idxFeatures, bool enforceJointCompatibility)
{
    ...
    for (int j = 0; j < nL; ++j)
    {
        ...
        std::cout << "Predicted feature " << j << " mean: " << featureDensity.mean().transpose() 
                  << ", covariance: \n" << featureDensity.cov() << std::endl;
                  Eigen::Vector2d predicted = featureDensity.mean();
    Eigen::Matrix2d cov = featureDensity.cov();
    
    for (int i = 0; i < m; ++i) {
        Eigen::Vector2d detected = Y.col(i);
        Eigen::Vector2d diff = detected - predicted;
        double mahaDist = std::sqrt(diff.transpose() * cov.inverse() * diff);
        std::cout << "Landmark " << j << " to Feature " << i << " Mahalanobis distance: " << mahaDist << std::endl;
    }
...
bool jointCompatibility(const std::vector<int> & idx, const double & sU, const Eigen::Matrix<double, 2, Eigen::Dynamic> & Y, const GaussianInfo<double> & density, const double & nSigma, double & surprisal)
{
    ...
    if (nA > 0)
    {
        // Measurement vector for associated landmarks
        Eigen::VectorXd yA = Y(Eigen::all, idxi).reshaped();
        GaussianInfo<double> marginal = density.marginal(idxyj);

        // Surprisal for unassociated landmarks plus surprisal for associated landmarks
        surprisal = nU*sU - marginal.log(yA);
        
        // Joint compatibility
        return marginal.isWithinConfidenceRegion(yA, nSigma);
    }
    else
    {
