...
struct QuadricPlot
{
    ...
    bool isInit;
    int landmarkId;
    void updateLabel(const std::string& text, const Eigen::Vector3d& color);
};
...
private:
    ...
    BasisPlot bp;
    ImagePlot ip;
    std::vector<Eigen::Vector3d> cameraTrajectory;
    vtkSmartPointer<vtkActor> trajectoryActor;
    void updateTrajectory(const Eigen::Vector3d& newPosition);
};
