    ...
    , trajectoryActor(vtkSmartPointer<vtkActor>::New())
{
    ...
    trajectoryActor->GetProperty()->SetColor(1.0, 1.0, 0.0); // Yellow color
    trajectoryActor->GetProperty()->SetLineWidth(2);
    threeDimRenderer->AddActor(trajectoryActor);
...
//         int markerId = pSystem->getLandmarkId(i);  // Assuming this method exists
//         qp.updateLabel("ID: " + std::to_string(markerId), Eigen::Vector3d(r, g, b));
...
//     // Get current landmark IDs
//     std::vector<int> currentLandmarkIds;
//     for (std::size_t i = 0; i < pSystem->numberLandmarks(); ++i) {
//         currentLandmarkIds.push_back(pSystem->getLandmarkId(i));
//     }

//     // Create a new vector to store the quadrics we want to keep
//     std::vector<QuadricPlot> newQpLandmarks;

//     // Iterate through existing quadrics
//     for (const auto& qp : qpLandmarks) {
//         if (std::find(currentLandmarkIds.begin(), currentLandmarkIds.end(), qp.landmarkId) != currentLandmarkIds.end()) {
//             // If the landmark still exists, keep it
//             newQpLandmarks.push_back(qp);
//         } else {
//             // If the landmark no longer exists, remove its actors from the renderer
//             threeDimRenderer->RemoveActor(qp.getActor());
//             threeDimRenderer->RemoveActor(qp.textActor);
//         }
//     }

//     // Swap the new vector with the old one
//     qpLandmarks.swap(newQpLandmarks);
...
//         // Find if this landmark already exists in our vector
...
//         {
//             // If this landmark doesn't exist in our vector, create it
...
//         if (camera.isWorldWithinFOV(landmarkPositionCV, Tnc))
//         {
//             GaussianInfo prQOi = pMeasurement->predictFeatureDensity(*pSystem, i);
//             //plotGaussianConfidenceEllipse(pSystem->view(), prQOi, rgb);
//         }

//         // Update 3D quadric plot with new position
...
//         // Update marker ID label
//         qp->updateLabel("ID: " + std::to_string(markerId), rgb);
...
void QuadricPlot::updateLabel(const std::string& text, const Eigen::Vector3d& color)
{
    ...
    // Position the label at the top of the quadric
    double centerX = (bounds.xmin + bounds.xmax) / 2.0;
    double centerY = (bounds.ymin + bounds.ymax) / 2.0;
    double centerZ = bounds.zmax + 0.1; // Slightly above the top of the quadric
    textActor->SetPosition(centerX, centerY, centerZ);
...
void Plot::updateTrajectory(const Eigen::Vector3d& newPosition)
{
    cameraTrajectory.push_back(newPosition);

    vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
    for (const auto& pos : cameraTrajectory)
    {
    ...
    vtkPolyDataMapper::SafeDownCast(trajectoryActor->GetMapper())->SetInputConnection(lineSource->GetOutputPort());
}