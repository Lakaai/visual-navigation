    ...
    GIVEN("A camera with no lens distortion")
    {
        ...
        GIVEN("The positive horizontal image axis unit vector")
        {
            ...
            WHEN("Checking field of view")
            {
                ...
                THEN("Vector is outside field of view")
                {
                    std::cout << "Horizontal Field of View (hFOV): " << cam.hFOV << " degrees" << std::endl;
                    std::cout << "Vertical Field of View (vFOV): " << cam.vFOV << " degrees" << std::endl;
                    CHECK_FALSE(isWithinFOV);
                }
            }
        }
        ...
        GIVEN("An arbitrary world point and camera pose")
        {
            cv::Vec3d rPNn(0.141886338627215, 0.421761282626275, 0.915735525189067);

            Pose pose;
            pose.translationVector = cv::Vec3d(0.792207329559554, 0.959492426392903, 0.655740699156587);
            pose.rotationMatrix = cv::Matx33d
            (
            ...
            REQUIRE(cv::norm(pose.rotationMatrix.t()*pose.rotationMatrix - cv::Matx33d::eye(), cv::NORM_INF) < 100*std::numeric_limits<double>::epsilon()); // Must be orthogonal

            WHEN("Evaluating worldToVector")
            {
                cv::Vec3d uPCc = cam.worldToVector(rPNn, pose);
            ...
            {
                cv::Vec2d rQOi = cam.worldToPixel(rPNn, pose);

                THEN("worldToPixel matches the composition of worldToVector and vectorToPixel")
                {
                    cv::Vec2d rQOi_expected = cam.vectorToPixel(cam.worldToVector(rPNn, pose));
