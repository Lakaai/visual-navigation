...
SCENARIO("Fundamental matrix fit: All inliers")
{
    GIVEN("Point correspondences between two frames")
    {
        ...
        WHEN("Calling findFundamentalMat")
        {
            ...
            cv::Mat Fkkm1_cv = cv::findFundamentalMat(rQOikm1, rQOik, cv::FM_RANSAC, threshold, 0.99, mask);
            ...
            Eigen::Matrix<double, 3, Eigen::Dynamic> pkm1(3, rQOikm1.size());
            Eigen::Matrix<double, 3, Eigen::Dynamic> pk(3, rQOik.size());

            for (size_t i = 0; i < rQOikm1.size(); ++i)
            {
                pkm1.col(i) << rQOikm1[i].x, rQOikm1[i].y, 1.0;
                pk.col(i) << rQOik[i].x, rQOik[i].y, 1.0;
            }
            Eigen::Matrix3d Fkkm1;
            cv::cv2eigen(Fkkm1_cv, Fkkm1);

            // Calculate normalised epipolar lines
            Eigen::Matrix<double, 3, Eigen::Dynamic> lk = Fkkm1 * pkm1;
   

            // Normalize the epipolar lines correctly
            Eigen::Matrix<double, 3, Eigen::Dynamic> nlk(3, lk.cols());

            for (int i = 0; i < lk.cols(); ++i)
            {
                double norm = lk.col(i).head<2>().norm();
                nlk.col(i) = lk.col(i)/norm;
            }

            // Calculate epipolar error
            Eigen::RowVectorXd d(pk.cols());
            for (int k = 0; k < pk.cols(); ++k)
            {
                // Normalize the point
                Eigen::Vector3d npk = pk.col(k);

                // Compute the dot product
                d(k) = npk.dot(nlk.col(k));
            }
